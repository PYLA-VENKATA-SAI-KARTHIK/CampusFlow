...
os.environ["DATABASE_URL"] = (
    "postgresql+asyncpg://campusflow:campusflow_dev"
    "@localhost:5432/campusflow_dev"
)
...
# Initialize JWTManager for tests to avoid lifespan weirdness
...
async def setup_test_db(db_engine) -> AsyncGenerator[None, None]:
    ...
    # Remove all test data/schema after the test.
    async with db_engine.begin() as connection:
        await connection.run_sync(Base.metadata.drop_all)
